...
and perform combinational operations on them by writing {\tt a~*~b}, {\tt a~\%~m}, and {\tt a~>>~1}, but it 
appears that this is not the case. The timing violations require us to handle large integers in smaller chunks.
...
One of the major trade offs in our design is cycle period and number of cycles per encryption/decryption. 
 During our preliminary synthesis exploration, we have found that we cannot run the operation unpipelined on the FPGA: the 
maximum frequency we can support mapped to the device is 25Mhz. In week 5, we will investigate timing violations and
...
decryption modules will be passed by SceMi into the hardware. A SceMi testbench will
